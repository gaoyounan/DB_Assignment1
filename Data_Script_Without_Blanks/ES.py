...
i = 0
c = 0
file = open(file_name + str(c) + ".txt", 'w')
...
for row in range(len(df_merg.index)):
    if i > 49999:
        ...
        file = open(file_name + str(c) + ".txt", 'w')
        i = 0
    index_r = "{\"index\": {\"_index\": \"_buss\", \"_type\": \"_doc\", \"_id\": \"" + str(df_merg.iloc[row, 9]) + "\"}}"
    data_r = "{\"route_id\": \"" + df_merg.iloc[row, 0] + "\", \"trip_headsign\": \"" + str(df_merg.iloc[
        row, 1]).replace(" ", "_") + "\", \"service_id\": \"" + df_merg.iloc[row, 2] + "\", \"shape_id\": \"" + df_merg.iloc[
        ...
        df_merg.iloc[row, 7]) + "\" ,\"stop_sequence\": " + str(df_merg.iloc[row, 8]) + ",\"name_stop\": \"" + \
             str(df_merg.iloc[row, 5]).replace(" ", "_") + "\"}"
    file.write(index_r + "\n")
    file.write(data_r + "\n")
    i += 1